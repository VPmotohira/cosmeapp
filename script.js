document.addEventListener('DOMContentLoaded', () => {
    ...
    function createTaskItemHTML(item) { return `<li class="task-item" id="task-${item.id}"><div class="item-image-placeholder">画像</div><input type="checkbox" class="task-checkbox" data-item-id="${item.id}"><div class="task-details"><div class="item-name">${item.name}</div><div class="item-category">${item.category}</div><div class="item-days">使用 ${calculateDays(item.startDate)} 日目</div></div><button class="details-btn" data-item-id="${item.id}">詳細</button></li>`; }
